...
void ORB_SLAM3::NLLSSolver<D, T>::optimize(ModelType &model)
{
...
template <int D, typename T>
void ORB_SLAM3::NLLSSolver<D, T>::optimizeGaussNewton(ModelType &model)
{
  ...
  for (iter_ = 0; iter_ < n_iter_; ++iter_)
  {
    ...
    if (ORB_SLAM3::norm_max(x_) <= eps_)
      break;
  }
}

template <int D, typename T>
void ORB_SLAM3::NLLSSolver<D, T>::optimizeLevenbergMarquardt(ModelType &model)
{
  // Compute weight scale
  if (use_weights_)
    computeResiduals(model, false, true);
  ...
  for (iter_ = 0; iter_ < n_iter_; ++iter_)
  {
    ...
    do
    {
      ...
      if (rho_ > 0)
      {
        ...
        model = new_model;
        chi2_ = new_chi2;
        stop_ = ORB_SLAM3::norm_max(x_) <= eps_;
...
template <int D, typename T>
void ORB_SLAM3::NLLSSolver<D, T>::setRobustCostFunction(
...
template <int D, typename T>
void ORB_SLAM3::NLLSSolver<D, T>::setPrior(
    const T &prior,
...
template <int D, typename T>
void ORB_SLAM3::NLLSSolver<D, T>::reset()
{
  have_prior_ = false;
  chi2_ = 1e10;
  mu_ = mu_init_;
...
template <int D, typename T>
inline const double &ORB_SLAM3::NLLSSolver<D, T>::getChi2() const
{
  return chi2_;
}

template <int D, typename T>
inline const ORB_SLAM3::Matrix<double, D, D> &ORB_SLAM3::NLLSSolver<D, T>::getInformationMatrix() const
{
  return H_;
}
