...
namespace ORB_SLAM3
{
                        ...
                        if (numOptMatches >= nSim3Inliers)
                        {
                            ...
                            if (numProjOptMatches >= nProjOptMatches)
                            {
                                ...
                                for (MapPoint *pMPi : vpMatchedMP)
                                {
                                    ...
                                    if (index >= 0)
                                    {
                                        int coord_x = pKFi->mvKeys[index].pt.x;
                                        if (coord_x < min_x)
                                        {
                                            min_x = coord_x;
                                        }
                                        ...
                                        int coord_y = pKFi->mvKeys[index].pt.y;
                                        if (coord_y < min_y)
                                        {
                                            min_y = coord_y;
                                        }
                                                    ...
                                                    std::vector<MapPoint *> &vpMPs, std::vector<MapPoint *> &vpMatchedMPs)
    {
        ...
        for(MapPoint* pMPi : vpMatchedMPs)
        {
            ...
            if(index >= 0)
            {
                int coord_x = pCurrentKF->mvKeys[index].pt.x;
                if(coord_x < min_x)
                {
                    min_x = coord_x;
                }
                ...
                int coord_y = pCurrentKF->mvKeys[index].pt.y;
                if(coord_y < min_y)
                {
                    min_y = coord_y;
                }
    ...
    void LoopClosing::printReprojectionError(set<KeyFrame *> &spLocalWindowKFs, KeyFrame *mpCurrentKF, string &name)
    {
        ...
        for (KeyFrame *pKFi : spLocalWindowKFs)
        {
            ...
            for (int j = 0; j < vpMPs.size(); ++j)
            {
                ...
                cv::KeyPoint point_img = pKFi->mvKeys[j];
                cv::Point2f reproj_p;
                float u, v;
