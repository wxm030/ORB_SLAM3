...
namespace ORB_SLAM3
{
    ...
    void ORBmatcher::GetWarpAffineMatrix(
        ...
        MapPoint *mp,
        int level, const Sophus::SE3 &TCR, Eigen::Matrix2d &ACR)
    {
        cv::Mat Rcw = ref->GetPose().rowRange(0, 3).colRange(0, 3);
        cv::Mat tcw = ref->GetPose().rowRange(0, 3).col(3);
        cv::Mat pc = Rcw * mp->GetWorldPos() + tcw;

        const Eigen::Vector3d pt_ref = {pc.at<float>(0), pc.at<float>(1), pc.at<float>(2)};
        float depth = pt_ref[2];

        // 偏移之后的3d点，深度取成和pt_ref一致
        const Eigen::Vector3d pt_du_ref = ref->Pixel2Camera(
            px_ref + Eigen::Vector2d(WarpHalfPatchSize, 0) * ref->mvScaleFactors[level], depth);
        const Eigen::Vector3d pt_dv_ref = ref->Pixel2Camera(
            px_ref + Eigen::Vector2d(0, WarpHalfPatchSize) * ref->mvScaleFactors[level], depth);

        const Eigen::Vector2d px_cur = curr->World2Pixel(pt_ref, TCR);
        const Eigen::Vector2d px_du = curr->World2Pixel(pt_du_ref, TCR);
        const Eigen::Vector2d px_dv = curr->World2Pixel(pt_dv_ref, TCR);
    ...
    void ORBmatcher::WarpAffine(
        const Eigen::Matrix2d &ACR, const cv::Mat &img_ref,
        const Eigen::Vector2d &px_ref, const int &level_ref, const KeyFrame *ref,
        const int &search_level, const int &half_patch_size, uint8_t *patch)
    {
        ...
        const Eigen::Vector2d px_ref_pyr = px_ref / ref->mvScaleFactors[level_ref];
        for (int y = 0; y < patch_size; y++)
        {
            for (int x = 0; x < patch_size; x++, ++patch_ptr)
            {
                Eigen::Vector2d px_patch(x - half_patch_size, y - half_patch_size);
                px_patch *= ref->mvScaleFactors[search_level];
                const Eigen::Vector2d px(ARC * px_patch + px_ref_pyr);
    ...
    bool ORBmatcher::FindDirectProjection(
        KeyFrame *ref, Frame *curr,
        MapPoint *mp, Eigen::Vector2d &px_curr, int &search_level)
    {
        Eigen::Matrix2d ACR;
        int index = std::get<0>(mp->GetObservations()[ref]);
        cv::KeyPoint kp = ref->mvKeys[index];

        Eigen::Vector2d px_ref(kp.pt.x, kp.pt.y);
        Sophus::SE3 TCR = Converter::toSE3(curr->mTcw * ref->GetPose().inv());
